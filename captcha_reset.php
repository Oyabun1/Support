		...
		else 
		{
			switch($captcha_id_value)
			{
				...
				case 'core.captcha.plugins.recaptcha':
					echo '<p style="font-size: 1.2em;">CAPTCHA  is <em>ReCAPTCHA plugin</em> therefore unchanged<p>';
					break;
				// 3.0.x values	
				case 'phpbb_captcha_qa':
					$sql = 'UPDATE ' . CONFIG_TABLE . '
					SET config_value = "core.captcha.plugins.qa"
					WHERE config_name = "captcha_plugin"';
					$db->sql_query($sql);
					echo '<p style="font-size: 1.2em;">CAPTCHA updated to <em>3.1.x Q&A CAPTCHA Plugin</em><p>';
				...
				case 'phpbb_captcha_gd':
					...
					WHERE config_name = "captcha_plugin"';
					$db->sql_query($sql);
					echo '<p style="font-size: 1.2em;">CAPTCHA updated to <em>3.1.x GD Image</em><p>';
					// Config value is cached so we'll clear that.
					$cache->purge();
					break;
				case 'phpbb_captcha_gd_wave':
					$sql = 'UPDATE ' . CONFIG_TABLE . '
					SET config_value = "core.captcha.plugins.gd_wave"
					WHERE config_name = "captcha_plugin"';
					$db->sql_query($sql);
					echo '<p style="font-size: 1.2em;">CAPTCHA updated to <em>3.1.x GD 3D Image</em><p>';
					// Config value is cached so we'll clear that.
					$cache->purge();
					break;
				case 'phpbb_captcha_nogd':
					...
					WHERE config_name = "captcha_plugin"';
					$db->sql_query($sql);
					echo '<p style="font-size: 1.2em;">CAPTCHA updated to <em>3.1.x Simple Image</em><p>';
					/// Config value is cached so we'll clear that.
					$cache->purge();
					break;
				case 'phpbb_recaptcha':
					$sql = 'UPDATE ' . CONFIG_TABLE . '
					SET config_value = "core.captcha.plugins.recaptcha"
					WHERE config_name = "captcha_plugin"';
					$db->sql_query($sql);
					echo '<p style="font-size: 1.2em;">CAPTCHA updated to <em>3.1.x ReCAPTCHA plugin</em><p>';
					/// Config value is cached so we'll clear that.
					$cache->purge();
					break;				
